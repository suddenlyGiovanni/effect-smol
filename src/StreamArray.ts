...
export const fromChannel = <A, E, R>(
  channel: Channel.Channel<ReadonlyArray<A>, E, unknown, unknown, R>
): Stream<A, E, R> => {
...
export const toChannel = <A, E, R>(
  stream: Stream<A, E, R>
): Channel.Channel<ReadonlyArray<A>, E, unknown, unknown, R> => (stream as any).channel
 ...
 * @category constructors
 */
export const fromIteratorSucceed = <A>(iterator: Iterator<A>, maxChunkSize?: number): Stream<A> =>
...
export const fromIterable = <A>(iterable: Iterable<A>): Stream<A> =>
  suspend(() => fromIteratorSucceed(iterable[Symbol.iterator]()))
