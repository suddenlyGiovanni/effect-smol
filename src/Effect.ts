 ...
 * @category EffectExit
 */
export const EffectExitTypeId: unique symbol = Symbol.for(
  "effect/Effect/EffectExit",
)
 ...
 * @category EffectExit
 */
export type EffectExitTypeId = typeof TypeId
 ...
 * @since 3.4.6
 * @experimental
 * @category EffectCause
 */
export const EffectCauseTypeId = Symbol.for("effect/Effect/EffectCause")
 ...
 * @category EffectCause
 */
export type EffectCauseTypeId = typeof EffectCauseTypeId

/**
 * A `EffectCause` is a data type that represents the different ways a `Effect` can fail.
 *
 * **Details**
 *
 * `EffectCause` comes in three forms:
 *
 ...
 * @category EffectCause
 */
export type EffectCause<E> =
  | EffectCause.Die
  | EffectCause.Fail<E>
  | EffectCause.Interrupt
 ...
 * @category guards
 */
export const isEffectCause = (self: unknown): self is EffectCause<unknown> =>
  hasProperty(self, EffectCauseTypeId)
 ...
 * @category EffectCause
 */
export declare namespace EffectCause {
  /**
   * @since 3.4.6
   * @experimental
   */
  export type Error<T> = T extends EffectCause.Fail<infer E> ? E : never
...
abstract class EffectCauseImpl<Tag extends string, E>
  extends globalThis.Error
  implements EffectCause.Proto<Tag, E>
{
  readonly [EffectCauseTypeId]: {
    _E: Covariant<E>
  }
  constructor(
    ...
    const causeName = `EffectCause.${_tag}`
    let name: string
    let message: string
    let stack: string
    ...
    super(message)
    this[EffectCauseTypeId] = microCauseVariance
    this.name = name
    this.stack = stack
  }
  pipe() {
...
class Fail<E>
  extends EffectCauseImpl<"Fail", E>
  implements EffectCause.Fail<E>
{
  constructor(
    readonly error: E,
...
export const causeFail = <E>(
  error: E,
  traces: ReadonlyArray<string> = [],
): EffectCause<E> => new Fail(error, traces)

class Die extends EffectCauseImpl<"Die", never> implements EffectCause.Die {
  constructor(
    readonly defect: unknown,
    traces: ReadonlyArray<string> = [],
  ) {
...
export const causeDie = (
  defect: unknown,
  traces: ReadonlyArray<string> = [],
): EffectCause<never> => new Die(defect, traces)

class Interrupt
  extends EffectCauseImpl<"Interrupt", never>
  implements EffectCause.Interrupt
{
...
export const causeInterrupt = (
  traces: ReadonlyArray<string> = [],
): EffectCause<never> => new Interrupt(traces)
...
export const causeIsFail = <E>(
  self: EffectCause<E>,
): self is EffectCause.Fail<E> => self._tag === "Fail"
 ...
 * @category EffectCause
 */
export const causeIsDie = <E>(self: EffectCause<E>): self is EffectCause.Die =>
  self._tag === "Die"
...
export const causeIsInterrupt = <E>(
  self: EffectCause<E>,
): self is EffectCause.Interrupt => self._tag === "Interrupt"
 ...
 * @category EffectCause
 */
export const causeSquash = <E>(self: EffectCause<E>): unknown =>
...
export const causeWithTrace: {
  (trace: string): <E>(self: EffectCause<E>) => EffectCause<E>
  <E>(self: EffectCause<E>, trace: string): EffectCause<E>
} = dual(2, <E>(self: EffectCause<E>, trace: string): EffectCause<E> => {
 ...
 * @category Fiber
 */
export const FiberTypeId = Symbol.for("effect/Effect/Fiber")
...
const fiberMiddleware = globalValue("effect/Effect/fiberMiddleware", () => ({
  interruptChildren: undefined as
    | ((fiber: FiberImpl) => Effect<void> | undefined)
    | undefined,
}))
...
const makePrimitiveProto = <Op extends string>(options: {
  ...
  readonly eval?: (
    fiber: FiberImpl,
  ) => Primitive | Effect<any, any, any> | Yield
  readonly contA?: (
    this: Primitive,
    value: any,
    fiber: FiberImpl,
...
>(options: {
  ...
  readonly prop: Prop
  readonly eval: (
    this: EffectExit<unknown, unknown> & { [args]: Parameters<Fn>[0] },
    fiber: FiberImpl<unknown, unknown>,
  ) => Primitive | Yield
}): Fn => {
  const Proto = {
    ...makePrimitiveProto(options),
    [EffectExitTypeId]: EffectExitTypeId,
    _tag: options.op,
...
/**
 * Creates a `Effect` effect that will succeed with the specified constant value.
 *
 * @since 3.4.0
 * @experimental
 * @category constructors
 */
export const succeed: <A>(value: A) => Effect<A> = makeExit({
  op: "Success",
  prop: "value",
  eval(fiber) {
...
/**
 * Creates a `Effect` effect that will fail with the specified `EffectCause`.
 *
 * @since 3.4.6
 * @experimental
 * @category constructors
 */
export const failCause: <E>(cause: EffectCause<E>) => Effect<never, E> =
  makeExit({
    op: "Failure",
    prop: "cause",
    eval(fiber) {
...
/**
 * Creates a `Effect` effect that fails with the given error.
 *
 * This results in a `Fail` variant of the `EffectCause` type, where the error is
...
/**
 * Creates a `Effect` effect that succeeds with a lazily evaluated value.
 *
 * If the evaluation of the value throws an error, the effect will fail with a
 * `Die` variant of the `EffectCause` type.
 *
 * @since 3.4.0
 * @experimental
 * @category constructors
 */
export const sync: <A>(evaluate: LazyArg<A>) => Effect<A> = makePrimitive({
  op: "Sync",
...
/**
 * Lazily creates a `Effect` effect from the given side-effect.
 *
 * @since 3.4.0
 * @experimental
 * @category constructors
...
/**
 * Pause the execution of the current `Effect` effect, and resume it on the next
 * scheduler tick.
 *
 * @since 3.4.0
 * @experimental
 * @category constructors
 */
export const yieldNowWith: (priority?: number) => Effect<void> = makePrimitive({
  op: "Yield",
  eval(fiber) {
    let resumed = false
 ...
 * @category constructors
 */
export const yieldNow: Effect<void> = yieldNowWith(0)

/**
 * Creates a `Effect` effect that will succeed with the value wrapped in `Some`.
 *
 * @since 3.4.0
 * @experimental
 * @category constructors
 */
export const succeedSome = <A>(a: A): Effect<Option.Option<A>> =>
  succeed(Option.some(a))

/**
 * Creates a `Effect` effect that succeeds with `None`.
 *
 * @since 3.4.0
 * @experimental
 * @category constructors
 */
export const succeedNone: Effect<Option.Option<never>> = succeed(Option.none())

/**
 * Creates a `Effect` effect that will fail with the lazily evaluated `EffectCause`.
 *
 * @since 3.4.0
 * @experimental
 * @category constructors
...
export const failCauseSync = <E>(
  evaluate: LazyArg<EffectCause<E>>,
): Effect<never, E> => suspend(() => failCause(evaluate()))

/**
 * Creates a `Effect` effect that will die with the specified error.
 *
 * This results in a `Die` variant of the `EffectCause` type, where the error is
 ...
 * @category constructors
 */
export const die = (defect: unknown): Effect<never> => exitDie(defect)

/**
 * Creates a `Effect` effect that will fail with the lazily evaluated error.
 *
 * This results in a `Fail` variant of the `EffectCause` type, where the error is
 * tracked at the type level.
 *
 ...
 * @category constructors
 */
export const failSync = <E>(error: LazyArg<E>): Effect<never, E> =>
  suspend(() => fail(error()))

/**
 * Converts an `Option` into a `Effect` effect, that will fail with
...
): Effect<A, NoSuchElementException> =>
  option._tag === "Some"
    ? succeed(option.value)
    : fail(new NoSuchElementException({}))

/**
 * Converts an `Either` into a `Effect` effect, that will fail with the left side
 ...
 * @category constructors
 */
export const fromEither = <R, L>(either: Either.Either<R, L>): Effect<R, L> =>
  either._tag === "Right" ? succeed(either.right) : fail(either.left)

const void_: Effect<void> = succeed(void 0)
export {
  /**
   * A `Effect` effect that will succeed with `void` (`undefined`).
   *
   * @since 3.4.0
   * @experimental
   * @category constructors
...
export {
  /**
   * The `Effect` equivalent of a try / catch block, which allows you to map
...
/**
 * Wrap a `Promise` into a `Effect` effect.
 *
 * Any errors will result in a `Die` variant of the `EffectCause` type, where the
...
/**
 * Access the `EffectExit` of the given `Effect` effect.
 *
 * @since 3.4.6
 * @experimental
 * @category mapping & sequencing
...
/**
 * Replace the error type of the given `Effect` with the full `EffectCause` object.
 *
 * @since 3.4.0
 * @experimental
 * @category mapping & sequencing
...
export const sandbox = <A, E, R>(
  self: Effect<A, E, R>,
): Effect<A, EffectCause<E>, R> => catchAllCause(self, fail)
...
export const raceAll = <Eff extends Effect<any, any, any>>(
  ...
  withFiber((parent) =>
    async((resume) => {
      ...
      const causes: Array<EffectCause<any>> = []
      const onExit = (exit: EffectExit<any, any>) => {
        doneCount++
...
/**
 * The `EffectExit` type is used to represent the result of a `Effect` computation. It
 ...
 * @category EffectExit
 */
export type EffectExit<A, E = never> =
  | EffectExit.Success<A, E>
  | EffectExit.Failure<A, E>
 ...
 * @category EffectExit
 */
export declare namespace EffectExit {
  /**
   * @since 3.4.6
   * @experimental
   * @category EffectExit
   */
  export interface Proto<out A, out E = never> extends Effect<A, E> {
    readonly [EffectExitTypeId]: EffectExitTypeId
  }
 ...
 * @category EffectExit
 */
export const isEffectExit = (u: unknown): u is EffectExit<unknown, unknown> =>
  hasProperty(u, EffectExitTypeId)
 ...
 * `EffectCause` if the predicate fails.
 *
...
export const filterOrFailCause: {
  <A, B extends A, E2>(
    ...
    orFailWith: (a: NoInfer<A>) => EffectCause<E2>,
 ...
 * fall `EffectExit` of the effect to determine if the repetition should continue.
 *
 * @since 3.4.6
 * @experimental
 * @category repetition
...
export const repeatExit: {
  <A, E>(options: {
    while: Predicate<EffectExit<A, E>>
...
/**
 * Catch the full `EffectCause` object of the given `Effect` effect, allowing you to
...
export const catchAllCause: {
  <E, B, E2, R2>(
    f: (cause: NoInfer<EffectCause<E>>) => Effect<B, E2, R2>,
...
/**
 * Selectively catch a `EffectCause` object of the given `Effect` effect,
...
export const catchCauseIf: {
  <E, B, E2, R2, EB extends EffectCause<E>>(
    refinement: Refinement<EffectCause<E>, EB>,
    f: (cause: EB) => Effect<B, E2, R2>,
  ): <A, R>(
    self: Effect<A, E, R>,
  ) => Effect<A | B, Exclude<E, EffectCause.Error<EB>> | E2, R | R2>
  <E, B, E2, R2>(
    predicate: Predicate<EffectCause<NoInfer<E>>>,
    f: (cause: NoInfer<EffectCause<E>>) => Effect<B, E2, R2>,
  ): <A, R>(self: Effect<A, E, R>) => Effect<A | B, E | E2, R | R2>
  <A, E, R, B, E2, R2, EB extends EffectCause<E>>(
    self: Effect<A, E, R>,
    refinement: Refinement<EffectCause<E>, EB>,
    f: (cause: EB) => Effect<B, E2, R2>,
  ): Effect<A | B, Exclude<E, EffectCause.Error<EB>> | E2, R | R2>
  ...
  <A, E, R, B, E2, R2>(
    self: Effect<A, E, R>,
    predicate: Predicate<EffectCause<E>>,
    f: (cause: EffectCause<E>) => Effect<B, E2, R2>,
  ): Effect<A | B, E | E2, R | R2> =>
...
/**
 * Perform a side effect using the full `EffectCause` object of the given `Effect`.
 *
 * @since 3.4.6
 * @experimental
 * @category error handling
...
/**
 * Perform a side effect using if a `EffectCause` object matches the specified
 * predicate.
 *
 * @since 3.4.0
 * @experimental
...
export const tapErrorCauseIf: {
  ...
  <E, B, E2, R2>(
    predicate: (cause: NoInfer<EffectCause<E>>) => boolean,
    f: (a: NoInfer<EffectCause<E>>) => Effect<B, E2, R2>,
  ): <A, R>(self: Effect<A, E, R>) => Effect<A, E | E2, R | R2>
  <A, E, R, B, E2, R2, EB extends EffectCause<E>>(
    self: Effect<A, E, R>,
    refinement: Refinement<EffectCause<E>, EB>,
...
export const catchIf: {
  ...
  <A, E, R, A2, E2, R2>(
    ...
    catchCauseIf(
      self,
      (f): f is EffectCause.Fail<E> => causeIsFail(f) && predicate(f.error),
      (fail) => f(fail.error),
    ),
)
...
/**
 * Transform the full `EffectCause` object of the given `Effect` effect.
 *
 * @since 3.4.6
 * @experimental
 * @category error handling
...
export const mapErrorCause: {
  <E, E2>(
    f: (e: EffectCause<E>) => EffectCause<E2>,
 ...
 * added to the `traces` field of the `EffectCause` object.
 *
 * @since 3.4.0
 * @experimental
 * @category error handling
...
export const withTrace: {
  ...
  function generate(name: string, cause: EffectCause<any>) {
...
export const matchCauseEffect: {
  <E, A2, E2, R2, A, A3, E3, R3>(options: {
    readonly onFailure: (cause: EffectCause<E>) => Effect<A2, E2, R2>
...
export const matchCause: {
  <E, A2, A, A3>(options: {
    readonly onFailure: (cause: EffectCause<E>) => A2
...
export interface EffectScope {
  ...
  readonly addFinalizer: (
    finalizer: (exit: EffectExit<unknown, unknown>) => Effect<void>,
  ) => Effect<void>
...
export declare namespace EffectScope {
  ...
  export interface Closeable extends EffectScope {
    readonly close: (exit: EffectExit<any, any>) => Effect<void>
  }
}
...
class EffectScopeImpl implements EffectScope.Closeable {
  ...
  state:
    | {
        readonly _tag: "Open"
        readonly finalizers: Set<(exit: EffectExit<any, any>) => Effect<void>>
      }
    | {
        readonly _tag: "Closed"
        readonly exit: EffectExit<any, any>
  ...
  unsafeAddFinalizer(
    finalizer: (exit: EffectExit<any, any>) => Effect<void>,
  ): void {
  ...
  close(microExit: EffectExit<any, any>): Effect<void> {
    return suspend(() => {
      if (this.state._tag === "Open") {
  ...
  get fork() {
    return sync(() => {
      ...
      function fin(exit: EffectExit<any, any>) {
...
export const acquireRelease = <A, E, R>(
  ...
  release: (a: A, exit: EffectExit<unknown, unknown>) => Effect<void>,
 ...
 * `EffectExit` of the executed effect.
 *
 * @since 3.4.6
 * @experimental
 * @category resources & finalization
...
export const onExit: {
  <A, E, XE, XR>(
    f: (exit: EffectExit<A, E>) => Effect<void, XE, XR>,
...
export const onExitIf: {
  <A, E, XE, XR, B extends EffectExit<A, E>>(
    refinement: Refinement<EffectExit<A, E>, B>,
    f: (exit: B) => Effect<void, XE, XR>,
  ): <R>(self: Effect<A, E, R>) => Effect<A, E | XE, R | XR>
  <A, E, XE, XR>(
    predicate: Predicate<EffectExit<NoInfer<A>, NoInfer<E>>>,
    f: (exit: EffectExit<NoInfer<A>, NoInfer<E>>) => Effect<void, XE, XR>,
  ): <R>(self: Effect<A, E, R>) => Effect<A, E | XE, R | XR>
  <A, E, R, XE, XR, B extends EffectExit<A, E>>(
    self: Effect<A, E, R>,
    refinement: Refinement<EffectExit<A, E>, B>,
    f: (exit: B) => Effect<void, XE, XR>,
  ): Effect<A, E | XE, R | XR>
 ...
 * `EffectCause` of the executed effect.
 *
 * @since 3.4.6
 * @experimental
 * @category resources & finalization
...
export const onError: {
  <A, E, XE, XR>(
    f: (cause: EffectCause<NoInfer<E>>) => Effect<void, XE, XR>,
  ): <R>(self: Effect<A, E, R>) => Effect<A, E | XE, R | XR>
  <A, E, R, XE, XR>(
    self: Effect<A, E, R>,
    f: (cause: EffectCause<NoInfer<E>>) => Effect<void, XE, XR>,
...
export const acquireUseRelease = <Resource, E, R, A, E2, R2, E3, R3>(
  ...
  release: (a: Resource, exit: EffectExit<A, E2>) => Effect<void, E3, R3>,
...
export const forEach: {
  ...
  withFiber((parent) => {
    ...
    return async((resume) => {
      ...
      let result: EffectExit<any, any> | undefined = undefined
      let inProgress = 0
      let doneCount = 0
      let pumping = false
...
/**
 * Attempt to execute the `Effect` effect synchronously and return the `EffectExit`.
 *
 ...
 * @category execution
 */
export const runSyncExit = <A, E>(effect: Effect<A, E>): EffectExit<A, E> => {
