...
describe("Mailbox", () => {
  ...
  it.effect("offer dropping", () =>
    Effect.gen(function*() {
      ...
      assert.deepStrictEqual((yield* mailbox.takeAll)[0], [1, 2])
    }))
  ...
  it.effect("offer sliding", () =>
    Effect.gen(function*() {
      ...
      assert.deepStrictEqual((yield* mailbox.takeAll)[0], [4, 5])
    }))
  ...
  it.effect("end with take", () =>
    Effect.gen(function*() {
      ...
      assert.strictEqual(yield* mailbox.take.pipe(Effect.flip), Option.none())
      assert.strictEqual(yield* mailbox.await, void 0)
      assert.strictEqual(yield* mailbox.offer(10), false)
    }))
  ...
  it.effect("fail", () =>
    Effect.gen(function*() {
      ...
      const takeArr = Effect.map(mailbox.takeAll, ([_]) => _)
