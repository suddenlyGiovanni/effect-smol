 ...
 *   // Set maximum operations before yielding to 100
 *   yield* MaxOpsBeforeYield.set(100)
 *
 *   // Get current max ops setting
 *   const currentMax = yield* MaxOpsBeforeYield
 *   yield* Effect.log(`Max ops before yield: ${currentMax}`)
 *
 *   // Run a compute-intensive task that will yield frequently
 *   const intensiveTask = Effect.gen(function* () {
 *     let result = 0
 ...
 *   })
 *
 *   return yield* intensiveTask
 * })
 *
 * // Configure for high-performance scenarios
 * const highPerformanceProgram = Effect.gen(function* () {
 *   // Increase max ops for better performance (less yielding)
 *   yield* MaxOpsBeforeYield.set(10000)
 *
 ...
 *   // Lower max ops for more frequent yielding
 *   yield* MaxOpsBeforeYield.set(50)
 *
